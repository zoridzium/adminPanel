 ...
 * exactly this option (note for non-scrolling else, please use the
...
FixedColumns.prototype = /** @lends FixedColumns.prototype */{
	 ...
	 *      var fc = new $.fn.dataTable.fixedColumns( table );
	 *
	 *      // Resize the table container and then have FixedColumns adjust its scriptSettings....
	 *      $('#content').width( 1200 );
	 *      fc.fnRedrawLayout();
	 */
	 ...
	 * else and get the index position for the data in the main table.
	...
	"_fnConstruct": function ( oInit )
	{
		...
		/* Set up the DOM that we want for the fixed column scriptSettings grid */
		this._fnColCalc();
		this._fnGridSetup();
	...
	/**
	 * Calculate the column widths for the grid scriptSettings
	 *  @returns {void}
	 *  @private
	 */
	"_fnColCalc": function ()
	...
	/**
	 * Set up the DOM for the fixed column. The way the scriptSettings works is to create a 1x3 grid
	 * for the left column, the DataTable (for which we just reuse the scrolling element DataTable
	 * puts into the DOM) and the right column. In each of he two fixed column elements there is a
	 * grouping wrapper element and then a head, body and footer wrapper. In each of these we then
	 * place the cloned header, body or footer else. This effectively gives as 3x3 grid structure.
	 *  @returns {void}
	 *  @private
	 */
	...
	/**
	 * Style and position the grid used for the FixedColumns scriptSettings
	 *  @returns {void}
	 *  @private
	 */
	...
	/**
	 * Make a copy of the scriptSettings object for a header or footer element from DataTables. Note that
	 * this method will clone the nodes in the scriptSettings object.
	 *  @returns {Array} Copy of the scriptSettings array
	...
	"_fnClone": function ( oClone, oGrid, aiColumns, bAll )
	{
		...
		if ( bAll )
		{
			...
			/* Copy the DataTables scriptSettings cache for the header for our floating column */
		...
		else
		{
			 ...
			 * cloned cells, just copy the classes across. To get the matching scriptSettings for the
			...
			for ( i=0, iLen=aoCloneLayout.length ; i<iLen ; i++ )
			{
				for ( j=0, jLen=aoCloneLayout[i].length ; j<jLen ; j++ )
				{
					...
					// If jQuery utilities theming is used we need to copy those elements as well
